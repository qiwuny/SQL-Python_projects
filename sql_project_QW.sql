...
the member name. */
select distinct Facilities.name, Members.surname, Members.firstname
from Facilities
join Bookings
...
select *
  ...
  join Members
    on Bookings.memid = Members.memid
  where (membercost*Bookings.memid + guestcost) * Bookings.slot - maintenance - outlay < 1000
